/** @file
    ...
    Binary demodulators (PWM/PPM/Manchester/...) using a pulse data structure as
   input.
...
static bitbuffer_t bits;

static int account_event(r_device *device, bitbuffer_t *bits,
                         char const *demod_name) {
  // run decoder
  int ret = 0;
  int decodeStatus = 0;
  if (device->decode_fn) {
...
int pulse_demod_pcm(const pulse_data_t *pulses, r_device *device) {
  ...
  if ((device->short_width > 0 && s_short <= 0) ||
    ...
    fprintf(stderr,
            "samples_per_us %f s_short %d s_long %d s_reset %d s_gap %d  "
            "s_sync %d s_tolerance %d \n",
            samples_per_us, s_short, s_long, s_reset, s_gap, s_sync,
            s_tolerance);
  ...
  float f_short = device->short_width > 0.0
                      ? 1.0 / (device->short_width * samples_per_us)
                      : 0;
  float f_long = device->long_width > 0.0
                     ? 1.0 / (device->long_width * samples_per_us)
                     : 0;
  ...
  for (unsigned n = 0; s_short != s_long && n < pulses->num_pulses; ++n) {
    ...
    if (count >= min_count) {
      ...
      if (device->verbose > 1) {
        ...
        fprintf(stderr,
                "Exact bit width (in us) is %.2f vs %.2f (pulse width %.2f vs "
                "%.2f), %d bit preamble\n",
                to_us / f_long, to_us * s_long, to_us / f_short,
                to_us * s_short, count);
      }
    }
  }
  ...
  for (unsigned n = 0; s_short == s_long && n < pulses->num_pulses; ++n) {
    ...
    if (count >= min_count) {
      ...
      if (device->verbose > 1) {
        float to_us = 1e6 / pulses->sample_rate;
        fprintf(stderr,
                "Exact bit width (in us) is %.2f vs %.2f, %d bit preamble\n",
  ...
  for (unsigned n = 0; n < pulses->num_pulses; ++n) {
    // Determine number of high bit periods for NRZ coding, where bits may not
    // be separated
    ...
    if ((s_short != s_long) // Only for RZ coding
        && (abs(pulses->pulse[n] - s_short) >
            s_tolerance)) { // Pulse must be within tolerance

      // Data is corrupt
      if (device->verbose > 3) {
        fprintf(stderr,
                "bitbuffer cleared at %u: pulse %d, gap %d, period %d\n", n,
    ...
    if (((n == pulses->num_pulses - 1)  // No more pulses? (FSK)
        ...
        && (bits.bits_per_row[0] > 0 ||
            bits.num_rows > 1)) { // Only if data has been accumulated
...
int pulse_demod_pwm(const pulse_data_t *pulses, r_device *device) {
  ...
  for (unsigned n = 0; n < pulses->num_pulses; ++n) {
    ...
    if (((n == pulses->num_pulses - 1)  // No more pulses? (FSK)
      ...
      events += account_event(device, &bits, __func__);
      bitbuffer_clear(&bits);
    } else if (s_gap > 0 && pulses->gap[n] > s_gap && bits.num_rows > 0 &&
...
int pulse_demod_manchester_zerobit(const pulse_data_t *pulses,
  ...
  if (s_tolerance <= 0)
    s_tolerance = s_long / 4; // default tolerance is +-45% of a bit period
  ...
  // First rising edge is always counted as a zero (Seems to be hardcoded policy
  // for the Oregon Scientific sensors...)
  bitbuffer_add_bit(&bits, 0);

  for (unsigned n = 0; n < pulses->num_pulses; ++n) {
    // The pulse or gap is too long or too short, thus invalid
    if (s_tolerance > 0 && (pulses->pulse[n] < s_short - s_tolerance ||
      ...
      if (pulses->pulse[n] > s_short * 1.5 &&
          pulses->pulse[n] <= s_short * 2 + s_tolerance) {
        // Long last pulse means with the gap this is a [1]10 transition, add a
        // one
        bitbuffer_add_bit(&bits, 1);
      }
      bitbuffer_add_row(&bits);
    ...
    // Falling edge is on end of pulse
      ...
      // so this pulse start must be a data edge (falling data edge means bit =
      // 1)
      bitbuffer_add_bit(&bits, 1);
      time_since_last = 0;
    } else {
    ...
    if (abs(symbol[n] - s_short) < s_tolerance) {
      ...
      if (abs(symbol[++n] - s_short) > s_tolerance) {
        if (symbol[n] >= s_reset - s_tolerance) {
          ...
          n--;
        } else if (bits.num_rows > 0 &&
                   bits.bits_per_row[bits.num_rows - 1] > 0) {
          bitbuffer_add_row(&bits);
          /*
                              fprintf(stderr, "Detected error during
             pulse_demod_dmc(): %s\n", device->name);
          */
        }
      }
...
int pulse_demod_piwm_raw(const pulse_data_t *pulses, r_device *device) {
  ...
  for (n = 0; n < pulses->num_pulses * 2; ++n) {
    ...
    } else if (symbol[n] < s_reset && bits.num_rows > 0 &&
               bits.bits_per_row[bits.num_rows - 1] > 0) {
      bitbuffer_add_row(&bits);
      /*
                  fprintf(stderr, "Detected error during pulse_demod_piwm_raw():
         %s\n", device->name);
      */
    }
...
int pulse_demod_piwm_dc(const pulse_data_t *pulses, r_device *device) {
  ...
  for (n = 0; n < pulses->num_pulses * 2; ++n) {
    if (abs(symbol[n] - s_short) < s_tolerance) {
      ...
      bitbuffer_add_row(&bits);
      /*
                  fprintf(stderr, "Detected error during pulse_demod_piwm_dc():
         %s\n", device->name);
      */
    }
