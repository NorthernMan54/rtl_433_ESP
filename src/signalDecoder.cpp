...
if (rtl_433_ESP::ookModulation) {
  ...
  memcpy(&cfg->devices[208], &wallarge_cltx001, sizeof(r_device));
  memcpy(&cfg->devices[209], &watchman_plus, sizeof(r_device));
  memcpy(&cfg->devices[210], &watts_thermostat, sizeof(r_device));
  memcpy(&cfg->devices[211], &watts_wfht_rf, sizeof(r_device));
  memcpy(&cfg->devices[212], &waveman, sizeof(r_device));
  memcpy(&cfg->devices[213], &wec2103, sizeof(r_device));
  memcpy(&cfg->devices[214], &wg_pb12v1, sizeof(r_device));
  memcpy(&cfg->devices[215], &ws2032, sizeof(r_device));
  memcpy(&cfg->devices[216], &wssensor, sizeof(r_device));
  memcpy(&cfg->devices[217], &wt1024, sizeof(r_device));
  memcpy(&cfg->devices[218], &wt450, sizeof(r_device));
  memcpy(&cfg->devices[219], &X10_RF, sizeof(r_device));
  memcpy(&cfg->devices[220], &x10_sec, sizeof(r_device));
  memcpy(&cfg->devices[221], &yale_hsa, sizeof(r_device));
} else {
