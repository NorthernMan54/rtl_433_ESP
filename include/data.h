...
typedef struct data {
  struct data *next; /**< chaining to the next element in the linked list; NULL
                        indicates end-of-list */
  char *key;
  char *pretty_key; /**< the name used for displaying data to user in with a
                       nicer name */
  char *format;     /**< if not null, contains special formatting string */
  data_value_t value;
  data_type_t type;
  unsigned retain; /**< incremented on data_retain, data_free only frees if this
                      is zero */
} data_t;
...
/** Constructs a structured data object.
    ...
    data_make(
            ...
            "others",   "More data",    DATA_DATA, data_make("foo",
   DATA_DOUBLE, 42.0, NULL), "zoom",     NULL,           data_array(2,
   DATA_STRING, (char*[]){"hello", "World"}), "double",   "Double",
   DATA_DOUBLE, 10.0/3, NULL);
    ...
    @param pretty_key Pretty name for the key. Use "" if to omit pretty label
   for this field completely, or NULL if to use key name for it.
    @param ... Type and then value of the item to put in, followed by the rest
   of the key-type-values. The list is terminated with a NULL.
...
/** Constructs an array from given data of the given uniform type.
    ...
    @return The constructed data array object, typically placed inside a data_t
   or NULL if there was a memory allocation error.
*/
R_API data_array_t *data_array(int num_values, data_type_t type,
                               void const *ptr);
...
typedef struct data_output {
  void(R_API_CALLCONV *print_data)(struct data_output *output, data_t *data,
                                   char const *format);
  void(R_API_CALLCONV *print_array)(struct data_output *output,
                                    data_array_t *data, char const *format);
  void(R_API_CALLCONV *print_string)(struct data_output *output,
                                     const char *data, char const *format);
  void(R_API_CALLCONV *print_double)(struct data_output *output, double data,
                                     char const *format);
  void(R_API_CALLCONV *print_int)(struct data_output *output, int data,
                                  char const *format);
  void(R_API_CALLCONV *output_start)(struct data_output *output,
  ...
  int log_level; ///< the maximum log level (verbosity) allowed, more verbose
...
/** Setup known field keys and start output, used by CSV only.
    ...
    @param fields the list of fields to accept and expect. Array is copied, but
   the actual strings not. The list may contain duplicates and they are
   eliminated.
